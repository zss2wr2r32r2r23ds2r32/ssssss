...
Tuff Macro is free software: you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation, either version 3 of the License, or (at your option) any later version.
...
MacroState := 0
path := '"' A_AhkPath '" "' A_ScriptDir '\tuff.ahk"'
...
Loop
{
	...
	if (((MacroState = 2) && (((time - LastMainHeartbeat > 120) && (reason := "Macro Unresponsive Timeout!"))
		...
		Loop
		{
			while WinExist("tuff ahk_class AutoHotkey")
			...
			if (WinWait("Tuff ahk_class AutoHotkeyGUI", , 300) != 0)
			{
				Sleep 2000
				Send_WM_COPYDATA("Error: " reason "`nSuccessfully restarted macro!", "tuff ahk_class AutoHotkey")
				Sleep 1000
				LastRobloxWindow := LastStatusHeartbeat := LastMainHeartbeat := LastBackgroundHeartbeat := nowUnix()
				break
			}
