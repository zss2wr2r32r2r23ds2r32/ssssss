	...
	switch (name := params[1]), 0
	{
			...
			'
			{
				"embeds": [{
					...
					},
					{
						...
						"value": "Stop and reload Tuff Macro (``F3``)",
						"inline": true
					},
					{
						"name": "' commandPrefix 'pause",
						"value": "Pause/unpause Tuff Macro (``F2``)",
						"inline": true
					},
					{
						"name": "' commandPrefix 'start",
						"value": "Start Tuff Macro (``F1``)",
						"inline": true
					},
					{
		...
		if (hwnd := WinExist(window := Trim(SubStr(command.content, InStr(command.content, name)+StrLen(name)))))
		{
			...
			if (windowPID = natroPID)
				discord.SendEmbed("Cannot close Tuff Macro window!", 16711731, , , , id)
			else
			{
