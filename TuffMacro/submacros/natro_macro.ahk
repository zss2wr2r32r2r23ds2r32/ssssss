...
This file is part of Tuff Macro. Our source code will always be open and available.

Tuff Macro is free software: you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation,
either version 3 of the License, or (at your option) any later version.

Tuff Macro is distributed in the hope that it will be useful. This does not give you the right to steal sections from our code, distribute it under your own name, then slander the macro.

You should have received a copy of the license along with Tuff Macro. If not, please redownload from an official source.
*/
/*
...
ElevateScript() {
	try
		...
		if !A_IsAdmin {
			MsgBox "You must run Tuff Macro as administrator in this folder!`nIf you don't want to do this, move the macro to a different folder (e.g. Downloads, Desktop)", "Error", 0x40010
			ExitApp
		}
		; elevated but still can't write, read-only directory?
		MsgBox "You cannot run Tuff Macro in this folder!`nTry moving the macro to a different folder (e.g. Downloads, Desktop)", "Error", 0x40010
	}
	else
		file.Close()
}
...
nm_importConfig()
{
	...
	config["Settings"] := Map("GuiTheme", "Tuff"
		, "AlwaysOnTop", 0
		, "MoveSpeedNum", 28
		, "MoveMethod", "Cannon"
...
nm_AutoUpdateGUI(*)
{
	...
	UpdateGui.Add("Text", "x" 149-posW//2 " y40 +BackgroundTrans", "Tuff Macro v" VersionID " ⮕ ")
...
nm_MajorUpdateHelp(*)
{
	...
	. "However, in Tuff Macro, your Settings are guaranteed to be transferable to any new version, so that option remains enabled.`n`n"
...
nm_MPuffModeHelp(*){ ; disable auto harvest information for manual planters
	...
	- If you have set up a Discord webhook and would like a ping and screenshot of the planter when full grown, select Planter Progress in Tuff Status tab > Change Discord Settings.
	- When ready, either:
...
nm_ResetConfig(*){
	if (MsgBox("
	(
	Are you sure you want to reset ALL Tuff settings?
...
nm_DetectedApplicationHelp(*){ ; detected application information
	...
	IMPORTANT:
	Tuff Macro currently does not support UWP Roblox due to a recent update!
...
nm_AutoStartManager(*){
	...
	if A_IsAdmin
		MsgBox "
		(
		Tuff Macro has been run as administrator!
		Auto-Start Manager can only launch Tuff Macro on logon without admin privileges.

		If you need to run Tuff Macro as admin, either:
		- fix the reason why admin is required (reinstall Roblox unelevated, move Tuff Macro folder)
	...
	if !(task := RegRead("HKCU\Software\Microsoft\Windows\CurrentVersion\Run", "TuffMacro", ""))
	...
	ASMGui.Add("Text", "x0 yp+34 vNTLabel", "Tuff Macro Path: ")
...
AddButton(*)
{
	...
	if (task := RegRead("HKCU\Software\Microsoft\Windows\CurrentVersion\Run", "TuffMacro", ""))
		if (MsgBox("Are you sure?`nThis will overwrite the existing Tuff Macro auto-start!", "Overwrite Existing Entry", 0x40024 " T30 Owner" ASMGui.Hwnd) != "Yes")
			return
	...
	RegWrite '"' A_WorkingDir '\START.bat"'
		...
		, "REG_SZ", "HKCU\Software\Microsoft\Windows\CurrentVersion\Run", "TuffMacro"
	...
	CenterText(ASMGui["StatusLabel"], ASMGui["StatusVal"], ASMGui["StatusLabel"])
	ASMGui["StatusText"].SetFont("cGreen"), ASMGui["StatusText"].Text := "Tuff Macro will automatically start on user login using the settings below:"
...
RemoveButton(*)
{
	...
	try RegDelete "HKCU\Software\Microsoft\Windows\CurrentVersion\Run", "TuffMacro"
	catch
	{
		; show msgbox
	}
	else
	{
		...
		CenterText(ASMGui["StatusLabel"], ASMGui["StatusVal"], ASMGui["StatusLabel"])
		ASMGui["StatusText"].SetFont("cRed"), ASMGui["StatusText"].Text := "No Tuff Macro auto-start found!`nUse the 'Add' button below."
...
nm_copyDebugLog(param:="", *) {
	...
	MacroInfo(){
		...
		(
			...
			point("Tuff Version", VersionID ((VerCompare(VersionID, LatestVer) < 0) ? ' (outdated)' : ''))	
	...
	DetectedProblems(){
		...
		return (
			...
			checkProblem((VerCompare(VersionID, LatestVer) < 0), 'Outdated Tuff Macro version')
			checkProblem((InStr(EnvGet("SESSIONNAME"), "RDP") && remoteDesktopMinimize != 2), 'Minimizing remote desktop connection will cause Tuff Macro to break')
			checkProblem(((DllCall("GetSystemMetrics", "int", 94)) & 0x40 && DllCall("GetSystemMetrics", "int", 95) >= 2), 'Touchscreen is enabled')
			checkProblem((robloxtype = RobloxTypes.UWP), 'Using UWP Roblox, it is currently unsupported for this Tuff Macro version')
...
start(*){
	...
	if !ForceStart {
		...
		if RemoteStart && (robloxtype = RobloxTypes.UWP || robloxtype = RobloxTypes.NotFound) {
			...
			if robloxtype = RobloxTypes.UWP {
				MsgBox "UWP Roblox installation is detected, Tuff Macro currently does not support it.`nPlease install Roblox from https://www.roblox.com/download", "UWP Roblox Detected", 0x40010 " T60"
				return UnlockStartButton()
			}
	...
	if !ForceStart {
		...
		try PostMessage 0x100, 0x7, 0, , "ahk_id " hRoblox
			...
			if !RemoteStart 
				...
				You must either reinstall Roblox without administrative rights, or run Tuff Macro as admin!
