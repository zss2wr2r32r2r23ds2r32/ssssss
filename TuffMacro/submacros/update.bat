...
if [%1]==[] (
    echo %red%This script must be run from Tuff Macro^^!%reset%
...
:: extract from %temp%/zip to Tuff Macro directory
...
(if exist "%~2\" (
    ...
    :: update autostart
    for /f "usebackq tokens=2,* skip=2" %%l in (`reg query "HKCU\Software\Microsoft\Windows\CurrentVersion\Run" /v "TuffMacro" 2^>nul`) do set "cmdline=%%m"
    if not [!cmdline!] == [] (
        call set strtest=%%cmdline:%~2=%%
        if not "!strtest!"=="!cmdline!" (
            call set cmdline=%%cmdline:%~2=!folder!%%
            call set regcmd=%%cmdline:"=\"%%
            reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Run" /v "TuffMacro" /d "!regcmd!" /f > nul
    ...
    echo %red%Error: Previous Tuff Macro folder not found^^!%reset%
...
echo %green%Update complete^^! Starting Tuff Macro in 10 seconds.%reset%
<nul set /p =%green%Press any key to skip . . . %reset%
timeout /t 10 >nul

start "" "!folder!\submacros\AutoHotkey32.exe" "!folder!\submacros\tuff.ahk"
exit)
