...
if exist "submacros\natro_macro.ahk" (
	if exist "submacros\AutoHotkey32.exe" (
		if not [%~3]==[] (
			set /a "delay=%~3" 2>nul
			echo Starting Tuff Macro in !delay! seconds.
...
if "%exe_missing%" == "1" (
	...
	echo %red% 1. Disable any third-party antivirus software ^(or add the Tuff Macro folder as an exception^)%reset%
	...
	echo:
	echo %red%Note: Both Tuff Macro and AutoHotkey are safe and work fine with Microsoft Defender^^!%reset%
...
if not [!grandparent!] == [] (
	...
	if not [!zip!] == [] (
		...
		if not [!zip!] == [] (
			...
			cd %USERPROFILE%
			for %%a in ("Downloads","Downloads\Tuff Macro","Desktop","Documents","OneDrive\Downloads","OneDrive\Downloads\Tuff Macro","OneDrive\Desktop","OneDrive\Documents") do (
				if exist "%%~a\!zip!" (
...
echo %red%Unable to automatically extract Tuff Macro^^!%reset%
