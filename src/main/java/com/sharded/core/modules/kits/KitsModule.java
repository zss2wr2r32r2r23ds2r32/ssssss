...
public final class KitsModule extends Module implements CommandExecutor, TabCompleter {
    ...
    @Override protected void onEnable() {
        ...
        reloadResources(); guiHandler = new KitsGuiHandler(this); registerCommand("kits", this); registerCommand("kit", this);
    }
    @Override protected void onDisable() { if (database != null) database.close(); database = null; kitCache.clear(); guiHandler = null; }
    void reloadResources() {
        for (String f : List.of("config.yml","messages.yml","kits.yml")) syncJarResource(f);
        loadConfigs(); kitsYaml = YamlConfiguration.loadConfiguration(new File(moduleFolder(),"kits.yml")); reloadKitCache();
    }
    ...
    private KitDefinition loadYamlKit(String name, ConfigurationSection section) {
        ...
        return new KitDefinition(name.toLowerCase(Locale.ROOT), section.getString("display-name", name), section.getString("icon","CHEST"), section.getInt("slot",11),
                section.getLong("cooldown-seconds", config.getLong("default-cooldown-seconds",86400)), section.getString("permission","sharded.kits."+name), section.getStringList("lore"), items, false);
    }
    ...
    void saveLayout(Player player, String kitName, Map<Integer, ItemStack> items) {
        if (!player.hasPermission("sharded.kits.admin")) { send(player,"no-permission"); return; }
        database.saveKit(kitName, "CHEST", config.getLong("default-cooldown-seconds",86400), "sharded.kits."+kitName, KitsDatabase.serializeContents(items));
    ...
    @Override public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (command.getName().equalsIgnoreCase("kits")) {
            ...
            if (!player.hasPermission("sharded.kits.use")) { send(player,"no-permission"); return true; }
    ...
    private boolean handleCreate(CommandSender sender, String[] args) {
        if (!(sender instanceof Player player)) { send(sender,"players-only"); return true; }
        if (!player.hasPermission("sharded.kits.admin")) { send(player,"no-permission"); return true; }
    ...
    private boolean handleDelete(CommandSender sender, String[] args) {
        if (!sender.hasPermission("sharded.kits.admin")) { send(sender,"no-permission"); return true; }
    ...
    @Override public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 2 && (args[0].equalsIgnoreCase("delete")||args[0].equalsIgnoreCase("give")||args[0].equalsIgnoreCase("layout")))
            return TabCompleteHelper.filter(args[1], allKits().stream().map(KitDefinition::name).toList());
        if (args.length == 2 && args[0].equalsIgnoreCase("give")) return TabCompleteHelper.onlinePlayers(args[1]);
        return List.of();
    }
}
