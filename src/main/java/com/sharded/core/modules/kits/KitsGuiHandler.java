...
final class KitsGuiHandler {
    ...
    void openMain(Player player) {
        ...
        Inventory inv = Bukkit.createInventory(holder, size, Text.c(module.config().getString("gui.main.title", "&8Kits")));
    ...
    void openPreview(Player player, String kitName) {
        ...
        int size = module.config().getInt("gui.preview.size", 54);
        KitsGuiHolder holder = new KitsGuiHolder(MenuType.PREVIEW, kit.name(), false);
        Inventory inv = Bukkit.createInventory(holder, size, Text.c(module.config().getString("gui.preview.title","&8Preview").replace("%kit%", kit.displayName())));
        holder.inventory = inv;
    ...
    void openLayout(Player player, String kitName) {
        int size = module.config().getInt("gui.layout.size", 54);
        KitsGuiHolder holder = new KitsGuiHolder(MenuType.LAYOUT, kitName.toLowerCase(), true);
        Inventory inv = Bukkit.createInventory(holder, size, Text.c(module.config().getString("gui.layout.title","&8Layout").replace("%kit%", kitName)));
        holder.inventory = inv;
