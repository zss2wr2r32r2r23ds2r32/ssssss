...
/**
 * Async-friendly economy service with in-memory cache and debounced SQLite writes.
 */
public final class EconomyService {
    ...
    public boolean isFrozen(UUID uuid) {
        CacheEntry cached = cache.get(uuid);
        if (cached != null) return cached.frozen();
        synchronized (this) {
            cached = cache.get(uuid);
            if (cached != null) return cached.frozen();
            EconomyDatabase.AccountRow row = database.getAccount(uuid);
    ...
    private void upsertBatch(Map<UUID, PendingWrite> batch) {
        for (PendingWrite write : batch.values()) {
            ...
            if (balance == null && cached != null) balance = cached.balance();
            if (frozen == null && cached != null) frozen = cached.frozen();
    ...
    public void invalidateAll() {
        cache.clear();
        Iterator<Map.Entry<UUID, PendingWrite>> it = pending.entrySet().iterator();
        while (it.hasNext()) {
            it.next();
            it.remove();
        }
