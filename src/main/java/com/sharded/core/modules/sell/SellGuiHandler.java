...
final class SellGuiHandler {
    ...
    void openSell(Player player) {
        var cfg = module.config().getConfigurationSection("gui.sell");
    ...
    void handleClick(Player player, SellGuiHolder holder, int slot) {
        if (holder.type == MenuType.SELL && slot == module.config().getInt("gui.sell.confirm-slot", 49)) module.processSell(player, holder.inventory);
