...
final class ShopGuiHandler {
    ...
    void openMain(Player player) {
        ...
        if (catalog.sections().isEmpty()) { module.sendMessage(player,"no-sections"); return; }
        ...
        for (Map.Entry<String, ShopCatalog.Section> entry : catalog.sections().entrySet()) {
            ...
            if (!perm.isBlank() && !player.hasPermission(normalizePermission(perm))) continue;
    ...
    private static String normalizePermission(String perm) {
        if (perm.startsWith("shardedcore.") || perm.startsWith("lifestealcore.")) {
            return perm.replace("lifestealcore.", "shardedcore.");
        }
        return perm.isBlank() || perm.startsWith("shardedcore.") ? perm : "shardedcore." + perm;
    }
}
