package com.shardedcore.modules.team;

import com.shardedcore.ShardedCore;
import com.shardedcore.module.Module;
import org.bukkit.event.Listener;
import com.shardedcore.util.ColorUtil;
import com.shardedcore.util.EnglishInputUtil;
...
public final class TeamsModule extends Module implements Listener, CommandExecutor, TabCompleter {
    ...
    org.bukkit.configuration.file.FileConfiguration teamConfig() {
        return config;
    }
    ...
    String guiRaw(String key, String... replacements) {
        String click = config.getString("gui.click-footer", "&7Click");
        String clickConfirm = config.getString("gui.click-footer-confirm", "&aConfirm");
        String clickCancel = config.getString("gui.click-footer-cancel", "&cCancel");
        String clickCreate = config.getString("gui.click-footer-create", "&aCreate");
    ...
    List<String> guiRawList(Player player, String key, String... replacements) {
        return Text.applyPlaceholderList(player, guiRawList(key, replacements));
    }

    String guiRaw(Player player, String key, String... replacements) {
        return Text.applyPlaceholders(guiRaw(key, replacements), player);
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!player.hasPermission("shardedcore.command.team")) {
        ...
        return switch (sub) {
            ...
            case "pvp" -> {
                handlePvp(player, args.length > 1 ? args[1] : null);
                yield true;
            }
            default -> {
                send(player, "usage");
    ...
    private void sendInviteMessage(Player target, String teamName, String inviterName) {
        Component line = ColorUtil.parse(raw("invite-received", "%team%", teamName, "%player%", inviterName));
        Component click = ColorUtil.parse(raw("invite-click"))
                .clickEvent(ClickEvent.runCommand("/team accept"))
                .hoverEvent(HoverEvent.showText(ColorUtil.parse(raw("invite-hover"))));
    ...
    void handleEmergency(Player player) {
        ...
        for (TeamDatabase.Member member : database.getMembers(teamId)) {
            ...
            if (online != null && !online.equals(player)) online.sendMessage(ColorUtil.parse(msg));
        }
    ...
    public void onChat(AsyncChatEvent event) {
        ...
        plugin.getServer().getScheduler().runTask(plugin, () -> {
            for (TeamDatabase.Member member : database.getMembers(teamId)) {
                Player online = Bukkit.getPlayer(member.uuid());
                if (online != null) online.sendMessage(ColorUtil.parse(out));
            }
            for (int allyId : database.getAllies(teamId)) {
                for (TeamDatabase.Member member : database.getMembers(allyId)) {
                    ...
                    if (online != null) {
                        ...
                        online.sendMessage(ColorUtil.parse(Text.apply(allyFormat, "%player%", player.getName(), "%message%", message)));
                    }
                }
            }
        });
    ...
    public void onTeamDamage(EntityDamageByEntityEvent event) {
        if (!(event.getEntity() instanceof Player victim) || database == null) return;
        Player attacker = resolveAttacker(event.getDamager());
        if (attacker == null) return;
        Integer teamId = database.getTeamId(victim.getUniqueId());
        if (teamId == null || !teamId.equals(database.getTeamId(attacker.getUniqueId()))) return;
        if (!database.isPvpEnabled(teamId)) {
            event.setCancelled(true);
            send(attacker, "pvp-blocked");
        }
    }
    ...
    private void handleSetHome(Player player) {
        ...
        database.setHome(teamId, player.getLocation());
    ...
    private void handleDelHome(Player player) {
        ...
        if (database.getHome(teamId) == null) {
            send(player, "no-home");
            return;
        }
        database.clearHome(teamId);
        send(player, "home-deleted");
    }

    private void handleHome(Player player) {
        Integer teamId = requireTeam(player);
        if (teamId == null) return;
        TeamDatabase.TeamHome home = database.getHome(teamId);
        if (home == null) {
            send(player, "no-home");
            return;
        }
        ...
        if (world == null) {
            send(player, "home-world-missing");
            return;
        }
        Location target = new Location(world, home.x(), home.y(), home.z(), home.yaw(), home.pitch());
        int delay = config.getInt("home.teleport-delay-seconds", 3);
        String countdown = config.getString("home.countdown-actionbar", "&#FF00F8&lTEAM &8▷ &#FF00F8&n{seconds}s");
        String cancelled = config.getString("home.cancelled-actionbar", "&#FF00F8&lTEAM &8▷ &7Teleport cancelled.");
        player.closeInventory();
        teleportHelper.teleportDelayed(player, target, delay, countdown,
                p -> send(p, "home-teleported"),
                () -> com.shardedcore.util.MessageUtil.sendActionBar(player, plugin, cancelled));
    }

    private void handlePvp(Player player, String arg) {
        Integer teamId = requireTeam(player);
        if (teamId == null) return;
        if (!isLeader(player, teamId)) {
            send(player, "not-leader");
        ...
        boolean current = database.isPvpEnabled(teamId);
        if (arg == null || arg.isBlank()) {
            database.setPvpEnabled(teamId, !current);
            send(player, !current ? "pvp-enabled" : "pvp-disabled");
            broadcastTeam(teamId, raw(!current ? "pvp-enabled-broadcast" : "pvp-disabled-broadcast", "%player%", player.getName()));
            return;
        }
        boolean next = switch (arg.toLowerCase(Locale.ROOT)) {
            case "on", "enable", "true" -> true;
            case "off", "disable", "false" -> false;
            case "toggle" -> !current;
            default -> {
                send(player, "pvp-usage");
                yield current;
            }
        };
        if (next == current && !arg.equalsIgnoreCase("toggle")) {
            send(player, next ? "pvp-already-enabled" : "pvp-already-disabled");
            return;
        }
        database.setPvpEnabled(teamId, next);
        send(player, next ? "pvp-enabled" : "pvp-disabled");
        broadcastTeam(teamId, raw(next ? "pvp-enabled-broadcast" : "pvp-disabled-broadcast", "%player%", player.getName()));
    }
    ...
    private void broadcastTeam(int teamId, String message) {
        for (TeamDatabase.Member member : database.getMembers(teamId)) {
            Player online = Bukkit.getPlayer(member.uuid());
            if (online != null) online.sendMessage(ColorUtil.parse(message));
        }
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (!(sender instanceof Player player) || !player.hasPermission("shardedcore.command.team")) return List.of();
        if (args.length == 1) {
            return TabCompleteHelper.filter(List.of(
                    "create", "invite", "accept", "join", "kick", "leave", "disband", "delete", "promote", "demote",
                    "members", "stats", "info", "leaderboard", "emergency", "ally", "chat", "gui",
                    "sethome", "delhome", "home", "pvp"), args[0]);
        }
        if (args.length == 2) {
            String sub = args[0].toLowerCase(Locale.ROOT);
            ...
            if (sub.equals("pvp")) {
                return TabCompleteHelper.filter(List.of("on", "off", "toggle"), args[1]);
            }
            if (sub.equals("ally")) {
                List<String> names = database.listTeams().stream().map(TeamDatabase.Team::name).collect(Collectors.toList());
                names.add("accept");
                return TabCompleteHelper.filter(names, args[1]);
            }
