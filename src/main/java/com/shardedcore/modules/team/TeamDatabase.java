...
public final class TeamDatabase {
    ...
    public TeamDatabase(ShardedCore plugin, File folder) throws SQLException {
        ...
        try (Statement statement = connection.createStatement()) {
            ...
            statement.execute("""
                    ...
                    """);
            migrateTeamColumns(statement);
        }
    }

    private void migrateTeamColumns(Statement statement) throws SQLException {
        addColumnIfMissing(statement, "teams", "home_world", "TEXT");
        addColumnIfMissing(statement, "teams", "home_x", "REAL");
        addColumnIfMissing(statement, "teams", "home_y", "REAL");
        addColumnIfMissing(statement, "teams", "home_z", "REAL");
        addColumnIfMissing(statement, "teams", "home_yaw", "REAL DEFAULT 0");
        addColumnIfMissing(statement, "teams", "home_pitch", "REAL DEFAULT 0");
        addColumnIfMissing(statement, "teams", "pvp_enabled", "INTEGER NOT NULL DEFAULT 1");
    }
    ...
    public synchronized void setHome(int teamId, org.bukkit.Location location) {
        try (PreparedStatement ps = connection.prepareStatement("""
                UPDATE teams SET home_world = ?, home_x = ?, home_y = ?, home_z = ?, home_yaw = ?, home_pitch = ?
                WHERE id = ?
                """)) {
            ...
            plugin.getLogger().warning("Failed to set team home: " + e.getMessage());
        }
    }

    public synchronized void clearHome(int teamId) {
        try (PreparedStatement ps = connection.prepareStatement("""
                UPDATE teams SET home_world = NULL, home_x = NULL, home_y = NULL, home_z = NULL, home_yaw = 0, home_pitch = 0
                WHERE id = ?
                """)) {
            ps.setInt(1, teamId);
            ps.executeUpdate();
        } catch (SQLException e) {
            plugin.getLogger().warning("Failed to clear team home: " + e.getMessage());
        }
    }

    public synchronized TeamHome getHome(int teamId) {
        try (PreparedStatement ps = connection.prepareStatement(
                "SELECT home_world, home_x, home_y, home_z, home_yaw, home_pitch FROM teams WHERE id = ?")) {
            ps.setInt(1, teamId);
            try (ResultSet rs = ps.executeQuery()) {
                if (!rs.next()) return null;
                String world = rs.getString("home_world");
                if (world == null || world.isBlank() || rs.getObject("home_x") == null) return null;
                return new TeamHome(world, rs.getDouble("home_x"), rs.getDouble("home_y"), rs.getDouble("home_z"),
    ...
    public synchronized boolean isPvpEnabled(int teamId) {
        try (PreparedStatement ps = connection.prepareStatement("SELECT pvp_enabled FROM teams WHERE id = ?")) {
            ps.setInt(1, teamId);
            try (ResultSet rs = ps.executeQuery()) {
                return !rs.next() || rs.getInt("pvp_enabled") != 0;
            }
        } catch (SQLException e) {
            return true;
        }
    ...
    public synchronized void setPvpEnabled(int teamId, boolean enabled) {
        try (PreparedStatement ps = connection.prepareStatement("UPDATE teams SET pvp_enabled = ? WHERE id = ?")) {
            ps.setInt(1, enabled ? 1 : 0);
            ps.setInt(2, teamId);
            ps.executeUpdate();
        } catch (SQLException e) {
            plugin.getLogger().warning("Failed to set team pvp: " + e.getMessage());
        }
    }
