...
public final class OrdersModule extends Module implements CommandExecutor, TabCompleter, Listener {
    ...
    String formatMoney(long amount) { return Numbers.format(amount); }
    ...
    public void disable() {
        ...
        database = null; guiHandler = null; cleanup();
    }
    void reloadResources() {
        syncResource("modules/orders/sounds.yml", new File(moduleFolder, "sounds.yml"));
        syncResource("modules/orders/items/blacklisted.yml", new File(moduleFolder, "items/blacklisted.yml"));
        for (String g : List.of("order","your_orders","new_order","deliver","edit","cancel_order","item_select")) {
            File target = new File(moduleFolder, "gui/" + g + ".yml");
            syncResource("modules/orders/gui/" + g + ".yml", target);
            layouts.put(g, new OrdersGuiLayout(target));
        }
    ...
    private boolean handleAdmin(CommandSender sender, String[] args) {
        if (!sender.hasPermission("shardedcore.command.orderadmin")) { send(sender,"no-permission"); return true; }
    ...
    @Override public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (command.getName().equalsIgnoreCase("orderadmin")) {
            if (args.length == 1) return TabCompleteHelper.filter(List.of("list", "remove", "migrate"), args[0]);
            return List.of();
        }
        if (args.length == 1) return TabCompleteHelper.filter(List.of("mine", "new", "stats", "toggle", "help"), args[0]);
        return List.of();
    }
}
