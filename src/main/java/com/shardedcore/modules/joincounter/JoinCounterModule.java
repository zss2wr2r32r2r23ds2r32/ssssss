package com.shardedcore.modules.joincounter;

import com.shardedcore.ShardedCore;
import com.shardedcore.module.Module;
import org.bukkit.event.Listener;
import com.shardedcore.util.ColorUtil;
import com.shardedcore.util.LuckPermsHelper;
...
public final class JoinCounterModule extends Module implements Listener, CommandExecutor, TabCompleter {

    private JoinCounterDatabase database;

    public JoinCounterModule(ShardedCore plugin) {
        super(plugin, "join-counter");
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!sender.hasPermission("shardedcore.joincounter.admin")) {
    ...
    private boolean handleJoinToggle(CommandSender sender, String[] args) {
        ...
        if (!player.hasPermission("shardedcore.command.jointoggle")) {
    ...
    private void broadcast(String template, Player player, long count, String type) {
        ...
        var component = ColorUtil.parse(message);
    ...
    private String applyPlaceholders(String message, Player player, long count) {
        return Text.apply(message,
                ...
                "%rank%", LuckPermsHelper.prefix(player),
                "%group%", LuckPermsHelper.primaryGroup(player));
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (cmd.equals("jointoggle")) {
            return args.length == 1 ? TabCompleteHelper.filter(java.util.List.of("toggle"), args[0]) : List.of();
        }
        if (!sender.hasPermission("shardedcore.joincounter.admin")) return List.of();
        if (args.length == 1) return TabCompleteHelper.filter(java.util.List.of("reset","clear","set"), args[0]);
        return List.of();
    }
}
